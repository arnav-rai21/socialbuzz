...
export default function App() {
  ...
  function handleSwitchAccount() {
    setProfileMenuOpen(false);
    // Non-destructive: keep the current identity until a NEW sign-in is verified,
    // so dismissing/cooldown never strands the visitor. handleOneTapCredential
    // replaces the identity only on success.
    disableOneTapAutoSelect();
    promptOneTap(GOOGLE_CLIENT_ID, handleOneTapCredential, { autoSelect: false });
  }
  function handleEditDetails() {
    setProfileMenuOpen(false);
    setCurrentView('form'); // make sure the editable form is in view
  }
  // Explicit sign-in affordance for signed-out attendees → triggers the inline
  // One Tap prompt (no popup). auto_select lets a single returning session
  // resolve in one tap.
  function handleAttendeeSignIn() {
    promptOneTap(GOOGLE_CLIENT_ID, handleOneTapCredential);
  }
      ...
      <header className="flex-shrink-0 border-b border-slate-100 bg-white" style={{ paddingTop: 'var(--safe-top)' }}>
        <div className="flex min-h-[56px] items-center justify-between px-4 sm:px-5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {/* Signed-out attendee: app-styled Sign in (triggers inline One Tap, no popup) */}
            {!identity && !isAdminAuthenticated && (
              <button
                onClick={handleAttendeeSignIn}
                className="flex items-center gap-1.5 pl-2 pr-3 py-1.5 rounded-full border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:border-violet-300 cursor-pointer transition-all active:scale-95 text-[13px] font-semibold shadow-sm"
              >
